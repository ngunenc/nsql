...
class query_builder {
    ...
    private function validate_operator(string $operator): void {
        $validOperators = ['=', '>', '<', '>=', '<=', '<>', 'LIKE', 'IN', 'NOT IN', 'IS', 'IS NOT'];
     ...
     */
    private function prepare_param(string $column, $value): array {
        $param_name = $this->normalizeParameterName($column . '_' . $this->param_counter++);
